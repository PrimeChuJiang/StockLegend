...
func _run_phase(phase: Enums.WorldPhase, ctx: Dictionary) -> void:
	GameBus.world_phase_started.emit(phase)
	print("[WorldActor] 阶段开始: %s" % Enums.WorldPhase.keys()[phase])
	...
	GameBus.world_phase_ended.emit(phase)
	print("[WorldActor] 阶段结束: %s" % Enums.WorldPhase.keys()[phase])

## 揭示本回合的日程表事件，发出 event_revealed 信号。
## TODO: 从事件牌库按回合索引取牌并公开。
func _reveal_events(_ctx: Dictionary) -> void:
	print("[WorldActor]   → 揭示本回合日程事件（暂无数据，跳过）")
...
func _resolve_breaking(_ctx: Dictionary) -> void:
	print("[WorldActor]   → 检查突发事件（暂无数据，跳过）")
